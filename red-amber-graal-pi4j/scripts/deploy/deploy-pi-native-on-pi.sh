...
# confirmed to work. BlackRaspberry (4B, 4 GB RAM) can complete the build
# in ~5 minutes. This path is not viable on the Pi Zero 2 W (512 MB RAM).
#
# Prerequisites:
...
REMOTE_BUILD_DIR="/tmp/red-amber-graal-pi4j-native-build"
...
ssh blackraspberry "
    ...
    ${NATIVE_IMAGE_PI} \
        ...
        -cp ${REMOTE_JAR} \
        -H:Name=red-amber-graal-pi4j-native \
        dev.lofthouse.App
    mkdir -p ~/.local/bin
    mv red-amber-graal-pi4j-native ~/.local/bin/red-amber-graal-pi4j-native
